...
import com.vladshtuka.explainer.databinding.FragmentDictionaryUaBinding
...
@AndroidEntryPoint
class DictionaryUaFragment : Fragment() {

    private lateinit var binding: FragmentDictionaryUaBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(
            inflater,
            R.layout.fragment_dictionary_ua, container, false
        )
        initRecyclerView()
        setUpObservers()
        viewModel.getDictionariesFromJson(Constants.DICTIONARY_UA)

        return binding.root
    }

    private fun setUpObservers() {
        viewModel.dictionaryUaList.observe(viewLifecycleOwner) { dictionaryUaList ->
            dictionaryAdapter.submitList(dictionaryUaList)
        }
    }
    ...
    private fun initRecyclerView() {
        ...
        binding.dictionaryRecyclerViewUa.adapter = dictionaryAdapter
        binding.dictionaryRecyclerViewUa.addItemDecoration(
